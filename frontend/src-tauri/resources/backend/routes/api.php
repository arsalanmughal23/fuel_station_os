...
Route::prefix('v1')->name('api.v1.')->group(function () {
    Route::get('/health', function () {
        return response()->json(['status' => 'ok', 'message' => 'API v1 is running']);
    });
    ...
    Route::middleware(['auth:sanctum', 'throttle:api'])->group(function () {
        ...
        Route::prefix('database')->name('database.')->group(function () {
            Route::get('info', [App\Http\Controllers\DatabaseController::class, 'info'])->name('info');
            Route::post('backup', [App\Http\Controllers\DatabaseController::class, 'backup'])->name('backup');
            Route::post('restore', [App\Http\Controllers\DatabaseController::class, 'restore'])->name('restore');
            Route::get('backups', [App\Http\Controllers\DatabaseController::class, 'listBackups'])->name('backups.index');
            Route::get('backups/{filename}', [App\Http\Controllers\DatabaseController::class, 'downloadBackup'])->name('backups.show');
            Route::delete('backups/{filename}', [App\Http\Controllers\DatabaseController::class, 'deleteBackup'])->name('backups.destroy');
        });
    });
});
